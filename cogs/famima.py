    ...
    async def on_submit(self, interaction: Interaction):
        if len(self.member_code.value) == 0 or len(self.pin.value) == 0:
            await interaction.response.send_message(
                content="Did not provide member code/PIN", ephemeral=True
            )
            return
        ...
        async with aiohttp.ClientSession() as session:
            ...
            if not data["success"]:
                ...
                await interaction.response.send_message(
                    content=f"Could not log in to FamilyMart: {details}", ephemeral=True
                )
                return
            ...
            await interaction.response.send_message(
                content="Successfully logged in.", ephemeral=True
            )
...
class FamimaLoginView(ui.View):
    ...
    @ui.button(
        label="Log in to FamilyMart",
        style=ButtonStyle.blurple,
        custom_id="FamimaLoginView:Login",
    )
...
class FamimaGachaCog(commands.Cog):
    ...
    async def famima_logout(self, ctx: commands.Context):
        await self.bot.db.execute(
            "DELETE FROM famima_creds WHERE id = ?", (ctx.author.id,)
        )
    ...
    async def famima_auto(self, ctx: commands.Context):
        async with self.bot.db.execute(
            "SELECT * FROM famima_creds WHERE id = ?", (ctx.author.id,)
        ) as cursor:
    ...
    async def famima_roll_task(self):
        async with self.bot.db.execute(
            "SELECT * FROM famima_creds WHERE auto_roll_channel_id != NULL"
        ) as cursor:
    ...
    async def _famima_roll_internal(self, member_code: str, pin: str):
        async with aiohttp.ClientSession() as session:
            ...
            resp = await session.get(
                "https://fmv-game.retailhub.vn/assets/config/config.env.json"
            )
            ...
            if not auth_data["success"]:
                details = escape_markdown(
                    auth_data["error"]["details"] or auth_data["error"]["message"]
                )
            ...
            if not autogen_data["success"]:
                details = escape_markdown(
                    autogen_data["error"]["details"] or autogen_data["error"]["message"]
                )
            ...
            if not game_play_data["success"]:
                details = escape_markdown(
                    game_play_data["error"]["details"]
                    or game_play_data["error"]["message"]
                )
            ...
            if not spin_data["success"]:
                details = escape_markdown(
                    spin_data["error"]["details"] or spin_data["error"]["message"]
                )
